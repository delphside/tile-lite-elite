...
once by **time**. **The first two digits are the section number below**, so a
diagram and the text can be read together; **a third digit orders the parts
within one step** — `1.1.1`–`1.1.2` are things you do, `1.6.1`–`1.6.5` are what
one command does on your behalf. There is no §1.6.4 to look up, and there is not
meant to be. Both diagrams use the same numbers as each other.
