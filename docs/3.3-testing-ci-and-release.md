...
| route | |
| --- | --- |
| **Production Release** — anything built into the image, **and anything deployed to the production host** | takes a semver, and needs the full lap to production |
| **Repository Change** — anything else in the repository | live on `origin/main`, so it never deploys |
| **Other** — a console, a cloud resource, preview or rehearsal | whatever applying it takes, recorded in [4.9](4.9-delivery-log.md) |

**The production host is a production release** — D52. A file, a systemd unit or
a package landing on it takes a semver and the full lap, even where nothing in
`crates/**` changed; production cloud configuration does not.

**A delivery has one route, and work spanning several is split into a delivery
each.** Owner, 2026-09-17:
...
Each takes the shape its own route needs: the scripts are live at their merge,
the console change when somebody applies it, the application at its production
deployment. Sequencing them is a project decision, and they may be ordered so
the earlier ones support the later.

**The exception is documentation, which rides.** A release carrying document
changes stays one delivery with one semver milestone — they are on the project
branch, reviewed in the same diff, and live when the release is.

*This replaced a ranking — "a delivery takes the heaviest route it touches",
Production Release over Other over Repository Change. The ranking existed to
summarise a mixed delivery, and splitting removes the thing it summarised.
[4.8](4.8-artefacts.md#the-route-an-artefact-takes) holds the artefact table
the split is made from.*

#### Then the shape

**Four shapes cover what has happened so far.** They are a catalogue, not a
taxonomy: they do not cover every `Other` change, and they do not cover a project
that needs an interim branch to integrate several work packages before any
release branch exists — which #71 will. A change that fits none of them is not
wrong; the catalogue is incomplete.

**Three things decide the shape**, and only one of them is `Route`:

| | |
| --- | --- |
| is the old version needed while the work is in progress? | if so it is held back on a branch, which then owes a pull request, a milestone and a row |
| does it go into the image? | `Route`, which is a summary of the routes of the artefacts it touches — [4.8](4.8-artefacts.md) |
| how many work packages ship together? | their shared milestone |

`Route` alone cannot decide it: `Repository Change` spans **A** and **B**, and
**A** spans two different reaches — a document is live at the merge, a console
change is not live until somebody applies it.

| | `Route` | milestone | branch | pull request | delivery log |
| --- | --- | --- | --- | --- | --- |
| **A · straight to main** | Repository Change · Other | `pre-approved`, or none | none | none | no row |
| **B · repository change with a branch** | Repository Change | previous semver plus a letter | one delivery branch | yes | a row |
| **C · single-delivery release** | Production Release | a semver | one delivery branch | yes | a row |
| **D · multi-delivery release** | Production Release | a semver | a release branch, and one per delivery | one each, plus one for the release | a row |
...
**A delivers, and that is why it has no milestone rather than despite it.** Owner,
2026-09-09: *"there is always a delivery, because the change must get to main. But
if it is applied straight to main then it is assumed not to be a single point in
time, and so it does not have a milestone that fits in with the strict sequence of
milestones."* The branch is what makes a delivery a point in time; the milestone is
what places that point in the sequence. Neither applies to a change that simply
lands, so its record is the commit, referencing the requirement or project it
serves — possibly more than one.

**`pre-approved` is the exception that proves it**: it marks approval, not a
position, which is why it sits outside the sequence and why a change can carry it
without being a point in time.

**#353 was recorded the wrong way on 2026-09-09** and is the worked example. Four
diagrams and two documents went straight to `main` in `37131fd` — correctly, since
a document-only change never takes a branch — and were then given a letter
milestone `0.7.2b` and a delivery-log row, which is flavour **B** without the branch
or the pull request that make B what it is. The milestone and the row have been
removed; the commit is the record.

**A release is always C or D.** Never A, and never a Production Release
committed straight to `main` **unproven**: `main` is what gets deployed, so an
untested release commit there blocks every other release until it is proven or
removed. That rule is [3.6 §2.2](3.6-change-lifecycle.md#22-when-to-branch-and-what-pre-approved-means),
and it is why 1.9 sits *before* 1.6 in both.

**Proven and unshipped is a different thing, and since D55 it is expected.**
1.9 has always merged a branch into `main` only after 1.3 – 1.5 have passed
against its tip, so a change on `main` has already been previewed, user-tested
and rehearsed. What D55 changed is that **1.6 need not follow immediately**:
`main` accumulates proven deliveries and a later release ships them together.
The blocking hazard the sentence above describes is about *untested* commits,
and it is untouched.

**The difference between C and D is only where the merging happens.** One
delivery fast-forwards `main` directly; several are gathered on a release branch
...
**Since D55, C is the default and D is for a release that must stabilise while
`main` moves on.** The owner's reason is about what `main` already carries:
...
A release branch has to be kept current against a `main` moving constantly with
documentation and tooling — which is the whole of 1.8. Where several proven
deliveries can simply accumulate on `main`, that cost buys nothing. Reach for D
when a release is held while other work continues, or when several projects must
be proven as a set before any of them lands.

**A delivery is a sub-project** ([3.6](3.6-change-lifecycle.md)), so the branch
belongs to the work package rather than to the parent — a parent builds nothing
and takes no branch. A project with one delivery is its own package, which is
why C reads the same as it did before.

**D drawn as branches**, from `0.8.0`, the first release to use one:
...
---
gitGraph
   commit id: "0.8.0 bump"
   branch "release/0.8.0"
   checkout main
   branch "297-lockfile"
   commit id: "Cargo.lock" type: HIGHLIGHT
   checkout main
   branch "301-user-id"
   commit id: "the labels" type: HIGHLIGHT
   commit id: "the messages" type: HIGHLIGHT
   commit id: "docs/4.3"
   checkout "release/0.8.0"
   merge "297-lockfile"
   merge "301-user-id"
   merge main tag: "sync, last"
   checkout main
   merge "release/0.8.0" tag: "prod-0.8.0"
```

**A square is a commit that changes the image**, a circle one that does not —
the `Production Release` and `Repository Change` rows of the delivery's own
artefact table. `docs/4.3` rides the branch because everything a delivery touches
goes on one branch, not because a document needed holding back.

**Every project branch comes off `main`, never off the release branch.** That is
what lets a package be descoped: cut the release branch again and merge only what
is still in scope. Rebasing one project onto another puts its commits in the
other's history, and untangling them afterwards is the expensive part.

**The merges are merges.** Each project's commit lands unchanged, so the commit
CI passed is the commit on the branch — see §1.8.1 for what `--rebase` cost when
it was tried.

**`main` moves only twice**: once for the version bump before the branch is cut,
and once at the end, by fast-forward. It is an ancestor of the release branch
throughout, which is what makes that last step possible.

**Each flavour is drawn**, with the arrows carrying the section numbers above, so
a step can be followed between the text and the picture:

| | diagram |
| --- | --- |
| the shape common to all four | [release-flow](diagrams/release-flow.svg) |
| **A** · pre-approved change | [flavour-a-pre-approved](diagrams/flavour-a-pre-approved.svg) |
| **B** · repository change with a branch | [flavour-b-repository-change](diagrams/flavour-b-repository-change.svg) |
| **C** · single-delivery release | [flavour-c-single-delivery-release](diagrams/flavour-c-single-delivery-release.svg) |
| **D** · multi-delivery release | [flavour-d-multi-delivery-release](diagrams/flavour-d-multi-delivery-release.svg) |

Sources are the `.mmd` beside each; [diagrams/README](diagrams/README.md) has the
one command that re-renders them and the conventions they follow.
...
**Cut an emergency from the last released tag, never from `main`.** Since D55,
`main` accumulates release-route changes that have passed testing but have not
shipped, so deploying `main` in an emergency ships all of them alongside the
fix — and the person restoring service is in no position to audit what else is
there. Branch from `prod-X.Y.Z`, apply the one fix, deploy that.

This is what makes D55 safe rather than merely convenient. Nothing enforces it
yet; #387 R2 asks for the report that would make the accumulation visible, and
R3 for the enforcement.
...
**They differ in scope as well as in question, and they share a machine, which
is what makes them easy to confuse.** Owner, 2026-09-16:

> The whole release goes through rehearsal deployment (not the same as technical
> testing which just happens to use the same environment) and production
> deployment, with checks that each work package is working correctly.

| | scope | when |
| --- | --- | --- |
| **1.5.2 technical testing** | one change | before that change merges to `main` |
| **1.5.1 rehearsal deployment** | the whole release | once, after `main` is assembled |

**A Release delivery is one deployment to production**, whatever it carries, and
one rehearsal deployment before it. Since D55 those happen after several changes
have each been user-tested and technically tested on their way to `main` — so
1.5.2 runs per change and 1.5.1 runs per release, which under the older shape
looked like one step because a release usually carried one delivery.

The post-deployment checks are per work package, because a check is answered per
delivery even where the deployment was shared.
...
**Write the equivalent for any change whose correctness depends on conditions
preview cannot reproduce.** Some changes have nothing a person can look at —
rate limiting is the worked example: preview shows that limiting happens, and
says nothing about the 2 vCPU box it protects. The judgement does not
disappear, it becomes a script and moves here.
...
**Every box says whose move it is** — `- [ ] **owner** — …` or
`- [ ] **Claude** — …`. Preview answers *does it do what was wanted*, which
needs a person in a browser, so those are the owner's; the rehearsal list is
proved by each script exiting 0 against the rehearsal host, so those are
Claude's. The label sits on the box rather than being read off the heading,
because a box that moves keeps its owner and a reader should not have to

**A third heading, `### Unit tests`, comes before the pair above and is not
part of their yes/no.** Owner, 2026-09-23: *"Unit tests (high level
descriptions, scenarios, don't list every field boundary test)... User
tests (Steve on Preview which require a browser and are not covered by
Unit tests. Steve will do his own user tests as well)."* Unlike Preview
and Rehearsal, every project has one: it is always present and always
Claude's, proved by the test suite passing rather than by a script against
a host or a person in a browser. Written at the level a reader needs to
judge whether the *coverage* is right — the scenarios covered, not a list
of every boundary case a test file already enumerates in its own test
names; that duplication is what the test run itself already answers. What
counts as adequate coverage, regardless of project, is
[2.4.4](#244-what-a-unit-test-is-expected-to-cover)'s.

**Preview's list only owes what a browser proves and Unit tests do not** —
the same economy of listing, the other direction. It is also not a
complete inventory of the owner's own testing: he explores beyond whatever
is written down, so the list is what needs naming in advance, not
everything he will actually do.

**Rehearsal is the default for a reason, and moving off it needs one too.**
Owner, 2026-09-23: *"Technical tests can be done elsewhere if there is a good
reason. But I don't think that applies here."* The table's own "run at"
column says why Rehearsal is the default — *production-like hardware* — so a
test run somewhere else is answering a narrower question than the heading
asks, and the body should say what narrower question it answered and why
that was enough.

**A faster local check made in passing while writing the real fix is not a
reason.** #399's technical-test box was briefly headed *"Technical tests —
local"*, standing in for the rehearsal run rather than supplementing it — the
local unit test proved the classification logic and proved nothing about
rehearsal's real infrastructure. Kept as an extra line under the Rehearsal
heading once corrected, not as a replacement for it.
scroll up to find out whose it is. `board-actions.py` (R1) is what reads them,
and an unlabelled box is reported by `board-check.py` rather than guessed at.
...
**It merges rather than rebases, and project branches stay on `main`.** Changed
2026-09-09, on the first release this flow ever ran. `gh pr merge --rebase`
rewrites the pull request's commits onto the base, which costs three things:

| | |
| --- | --- |
| the tested commit is not the landed commit | #338 was tested as `21d40b2` and landed as `fa25f7a`, with a different tree. Its run proved a commit that never existed on the release branch |
| the commit stamp breaks *after* the merge | `check-commit-stamp.sh` reads the version from the tree **at that commit**. A branch cut when `main` was `0.7.3` is self-consistent until a rebase drops it onto a `0.8.0` tree — and then it is wrong where no test can see it |
| project branches must sit on the release branch | only then is the rebase a no-op. That couples them: descoping one package means untangling it from every package rebased after it |

A merge preserves each commit's own tree, so the stamps survive, the tested SHA
is the landed SHA, and **each project branch is rebased onto `main` and left
there** — independent of every other, and descopable by recreating the release
branch without it.

**Recreating the release branch is the descope mechanism.** Owner, 2026-09-09:
otherwise it simply accumulates. Nothing else needs to happen to drop a package —
cut the branch again from `main` and merge only what is still in scope.

**The cost is a merge commit** on the release branch, and therefore on `main`
after 1.9. `--ff-only` is unaffected, because `main` remains an ancestor.
...
./scripts/document-map.py --write   # the map of this branch's headings; commit it if it changed
...
git checkout main
git merge --ff-only <branch>    # the post-merge hook moves the work package's Phase on
git push origin main
#   ... then 1.6 ship it, and 1.7, from `main` ...
```

**The merge moves the work package past testing by itself.** `.githooks/post-merge`
sees `git merge <N-branch>` on `main` and runs `scripts/board-merged.py`, which
sets a Production Release's Phase to *Deployment* (merged, waiting for the release
that ships it) and a Repository Change's to *Post-deployment* (the merge was its
delivery). It finds the package from the pull request, so a branch named for the
parent still moves the right one. It prints what it did; anything it could not do
is printed with what to set by hand.
...
```bash
./scripts/rollback.sh                       # 1. go back to what worked — seconds, no build

# 2. only if 1 cannot help. Cut from what production is running, not from main.
git switch --detach "$(git tag -l 'prod-*' --sort=-v:refname | head -1)"
git switch -c emergency-<what>
# ... the one fix, committed ...
git push -u origin emergency-<what>
DEPLOY_EMERGENCY="logins 500 since 14:20" ./scripts/deploy.sh emergency-<what>
```

**From the last `prod-*` tag, never from `main`** — #387 R3, decided 2026-09-20.
`main` accumulates tested-but-unshipped image changes (D55), and deploying it in
an emergency ships every one of them beside the fix, with nobody in a position
to audit what else went. Cutting from the tag makes the release the fix and
nothing else.

**This took 12 minutes 7 seconds when it was walked**, 2026-09-20, against
rehearsal: 4 seconds to branch, fix and push; **6 minutes of CI**; 5 minutes 40
to deploy. CI is half of it and is not skipped, which is why `rollback.sh` comes
first — it is seconds and needs no build.

**Afterwards, once things are calm**, `main` does not have the fix: the branch
was cut from a tag, so it sits on a line of its own.

```bash
git switch main && git merge emergency-<what>
```

**Merge, not rebase and not cherry-pick.** Three things key on commit shas —
`artifacts/<sha>.tar.gz` (#214), `verify.sh`'s `check_unreleased`, and the
deploy gate that requires the exact commit on the remote. A rebase rewrites
`main` and orphans every artefact built from it; a cherry-pick leaves production
running a commit that is not an ancestor of `main`, so `check_unreleased` reports
a change production already has. A merge makes the deployed commit reachable,
which is what all three already assume.

The milestone and its issues are not closed by an emergency deploy — close what
actually shipped by hand.
...
| | |
| --- | --- |
| `./scripts/board-status.py` | where every open change is, and what each environment is running |
...
#### 2.4.4 What a unit test is expected to cover

**Unit Test Coverage Checklist** —
[RIGHT-BICEP](https://www.oreilly.com/library/view/pragmatic-unit-testing/9781680500769/f_0051.html)
and its Boundary expansion,
[CORRECT](https://www.oreilly.com/library/view/pragmatic-unit-testing/9781680500769/f_0060.html),
from *[Pragmatic Unit Testing](https://media.pragprog.com/titles/utj/StandaloneSummary.pdf)*
(Langr, Koskela, Loyer), adopted rather than assembled ourselves. Not
every letter applies to every change — a data model has no performance
characteristic worth a test, most functions have no inverse — but the
checklist's job is the nudge to consider each one, not a requirement that
all seven fire:

- **Right** — the results are actually correct, not merely returned
- **Boundary** — CORRECT: **C**onformance to the expected format;
  **O**rdering; **R**ange (min, max, zero, negative); **R**eference to
  anything outside the code's own control; **E**xistence (non-null,
  non-zero, present); **C**ardinality (exactly enough values); **T**ime
  (ordering and timing)
- **Inverse** — relationships checked in reverse where one exists
  (encode/decode, save/load)
- **Cross-check** — the result checked against a second, independent way
  of arriving at it, where one exists
- **Error** conditions forced and confirmed handled, every path actually
  reached
- **Performance** characteristics checked where the code has one worth a
  test

When writing or conducting unit tests, ensure each of these categories is
covered. There is no need to list them for each project — it applies to
every change made here, so naming it per-project would be the same
sentence on every issue rather than something a reader learns from that
one.

**Not listed is not the same as not tested.** [1.5.2](#152-does-it-behave-correctly-on-production-like-hardware)'s
`### Unit tests` heading owes a project's Test approach a summary of the
scenarios covered, not this checklist spelled out again — the rule is
against naming these in an issue, not against covering them.

`.claude/skills/unit-test-checklist/` fires this checklist at the moment
it is needed: writing or reviewing a change's unit tests, which is not
reliably a moment this document is already open.
...
| `advisories.yml` | `schedule` at 06:15 UTC, and `workflow_dispatch`. Never on a push, because the advisory database changes and `Cargo.lock` mostly does not |
| `context.yml` | `schedule` at 06:30 UTC daily, and `workflow_dispatch`. Rewrites the context header on any open project whose facts changed — `scripts/board-context.py --write` |
| `programme-activities.yml` | `schedule` at 07:00 UTC on Mondays, and `workflow_dispatch` |
...
| **a status changed on a path that already existed** | **minor** |
| anything a client cannot observe on the wire | none |

**The third row was missing and the case has been missed twice.** #380 changed
what a database failure answers across the API and did not bump; #399 changed
the last two call sites and did not either, until a review caught it. The
precedent both should have followed is `ec72268`, which moved 2.10 to 2.11 for
*403 for a caller who is signed in but not allowed* — the same shape.

**It is minor rather than major even though a retrying client behaves
differently**, because an old client still works: it receives a status it
understands and a `Retry-After` it may ignore. Major is for a client that
...
**Moved to [3.6 The change lifecycle](3.6-change-lifecycle.md)** on 2026-08-21 —
...
Four layers, and the point is that they answer **different questions** rather
than the same one with increasing confidence:
...
| user testing | preview, step 1.4 | does it do what was actually wanted |
| the release mechanism | rehearsal, step 1.5 | does deploying it actually work |

**A step that describes something the system no longer does is worse than no
step, because it is followed.** That is the one thing about this document a
reader cannot check for themselves, and it is why 3.4 exists.
...
**One build per commit, shared by every environment.** The first deploy of a
commit builds `artifacts/<sha>.tar.gz` and records its digest beside it; every
later environment loads that file. So rehearsal proves the mechanism against
the **bytes** production will run, not against another build of the same
commit.

**Why that had to be the artefact rather than a version check.**
`TILE_LITE_ELITE_BUILD_ID` is the target SHA, so a rebuild stamps the *same*
version string — two builds of one commit report identical `/health` output
while running different bytes. A gate comparing versions is not a weak gate,
it is a blind one, which is why the gate compares digests. Base tags float
(`rust:1-bookworm`, `debian:bookworm-slim`, `caddy:2-alpine`), so two builds
minutes apart genuinely can differ.

**Do not replace the digest comparison with a version comparison.** It will
look equivalent and pass every test, because the failure it catches is one
...
**Moved to [3.6 The change lifecycle](3.6-change-lifecycle.md#3-notes-on-release-scope-and-version-numbers)**,
