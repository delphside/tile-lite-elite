...
**A change comes in one of four shapes**, and they are decided by fields
already recorded rather than by judgement: `Route`, the milestone, and how many
...
| **B · repository change with a branch** | Repository Change | previous semver plus a letter | one | yes | a row |
...
project fast-forwards `main` directly; several are gathered on a release
branch first, each merge gated by 1.8.1, and the branch tip is what is proven
and then fast-forwarded. Everything from 1.3 onwards is identical.
