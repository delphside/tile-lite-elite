...
`crates/**` changed; production cloud configuration does not. Such a change is
expected to **ride with a release already being planned** rather than cause one,
so the release carries host steps beside the image and the delivery's runbook
gives the order.
