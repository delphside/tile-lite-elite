...
**The excluded kind, concretely**: a numeric field's min, max, zero and
negative; a text field's max length and that Unicode displays correctly;
every error path actually being reached. These are owed by the test suite
regardless of the project, so naming them per-project would be the same
sentence on every issue rather than something a reader learns from.

**Not listed is not the same as not tested.** These are still written as
unit tests, on every field and every error path they apply to — the rule
is against naming them in an issue's Test approach, not against covering
them. What is missing is a test, not a mention of one.
