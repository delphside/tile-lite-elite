...
**Unit Test Coverage Checklist** — RIGHT-BICEP, from *Pragmatic Unit
Testing* (Langr, Koskela, Loyer), adopted rather than assembled ourselves.
Not every letter applies to every project — a data model has no
performance characteristic worth a test, most functions have no inverse —
but the checklist's job is the nudge to consider each one, not a
requirement that all seven fire:
