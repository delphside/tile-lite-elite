...
**A delivery takes the heaviest route it touches, not the first that fits**, and
the ranking is **Production Release, then Other, then Repository Change** — not
the table's order. A release also changes the repository, so *Repository Change*
means **anything else** in it; and `Other` outranks it, because something
applied by hand is not live until somebody applies it. See
[4.8](4.8-artefacts.md#the-route-an-artefact-takes).
