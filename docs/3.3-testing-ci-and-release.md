...
**A, B and C were all in use before 2026-09-07; D was not.** Release branches
arrived with #344, and until then a release carrying more than one project had
no documented shape at all.
...
branch's own push runs were green when `e2e` had not run (#144).

**Two `ci.yml` rules make this work**, both added by #344:
...
touching only `crates/` or `scripts/` got no run at all. #339 R3.
...
**The process leans on these, and several have caused defects.** Read off the
workflows rather than remembered.

**When a run happens, and which jobs are in it** — `.github/workflows/ci.yml`:

| | |
| --- | --- |
| triggers | `push` (**any** branch), `pull_request`, `workflow_dispatch` |
| `fmt · clippy · test · wasm · docs` | every one of those |
| `commit stamp (app/api versions)` | every one of those |
| `e2e (Playwright · preview stack)` | `pull_request`, `workflow_dispatch`, or a **push to `main` or `release/*`**. Needs the `check` job first |
| concurrency | group `ci-<ref>`. A newer run **cancels** an in-progress one, **except** on `main` and `release/*`, where it **queues** behind it instead |

**The documentation checks** — `.github/workflows/docs.yml`:

| | |
| --- | --- |
| triggers | `pull_request` (opened · edited · synchronize · reopened · ready_for_review), **filtered by path**: `docs/**`, `*.md`, `.markdownlint.jsonc`, `scripts/check-doc*`, or the workflow itself |
| `review-checklist` | that trigger, and not a draft |

**So a pull request touching none of those paths gets no `review-checklist` run
at all** — not a pass, no run. Every pull request so far has happened to touch
`docs/`, so it has never shown. Related to #339, which is the same check
passing when a body has no review heading.
