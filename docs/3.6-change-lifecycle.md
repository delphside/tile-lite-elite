...
**A work package is a sub-project; a delivery is a milestone.** D51, 2026-09-08,
refined 2026-09-08 after the two were found to be conflated. `Project Delivery`
was deleted the same day; it had one instance ever, carrying `Workstream` and
nothing else.

| | is | carries |
| --- | --- | --- |
| a **work package** | a unit of work — a `Project` whose parent is this one | its own milestone |
| a **delivery** | a set of work packages going live together, **identified by their shared milestone** | no issue of its own, ever |
| the **parent** | the owner of the requirements, the design and the documents | no milestone. It has no delivery role |

**A delivery never has an issue**, so it never appears in a title. Which work
packages ship together is read off the milestone, which is already maintained
because the deploy reads it — where a number written into a title would go stale
the moment two were grouped.

**The parent lists its deliveries**, each row naming the work packages it carries
or saying `pre-approved`. That list is what makes a delivery findable at all,
since it has no issue.
