...
**What decides the ceremony is whether a release can carry the change.** Not its
size, and not who raised it — those decide how much gets written down, not what
the change owes.

| update | reaches | how it reaches production | what it owes |
| --- | --- | --- | --- |
| Dependabot, GitHub Actions | CI only | never — it is not in the image | its commit message |
| Dependabot, Docker base images | the image | **the next release**, whenever that is | its commit message |
| a crate bump, advisory or otherwise | the image | **the next release** | its commit message |
| host packages, the container runtime | the host | applied to the host directly | a work package, letter milestone and delivery-log row |
| a kernel, or a reboot to adopt one | the host | applied to the host directly | the same |

**A repository-side update rides the next release and never justifies one.**
...
**A host-side update has no release to ride, and that is the whole asymmetry.**
